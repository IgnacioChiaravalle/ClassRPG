...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
	...
	<body>
		...
		<button title="Descartar Cambios y Volver" class="go-back-button" onclick="location.href='/my-account'"></button>
		...
		@csrf
			...
			<div>
				<input id="password" type="password" class="field {{old('password') ? 'active-field' : 'default-field'}}" name="password" required autofocus onkeypress="activateField(this); enableSubmitIfAllActive(2, 'password-change-submit')" onclick="activateField(this); enableSubmitIfAllActive(2, 'password-change-submit')">
				@error('password')
			...
			<div>
				<input id="password_confirmation" type="password" class="field {{old('password_confirmation') ? 'active-field' : 'default-field'}}" name="password_confirmation" required onkeypress="activateField(this); enableSubmitIfAllActive(2, 'password-change-submit')" onclick="activateField(this); enableSubmitIfAllActive(2, 'password-change-submit')">
			</div>
